...
public class TaskController {
    ...
    public ResponseEntity<Response> updateTask(@RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String token, @PathVariable Long id, @RequestBody Task task) {
        Response response = new Response();
        try {
            if (token == null || !token.startsWith("Bearer ")) {
                response.setMessage("Token no proporcionado o formato inválido");
    ...
    public ResponseEntity<Response> deleteTask(@RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String token,@PathVariable Long id) {
        Response response = new Response();
        try {
            if (token == null || !token.startsWith("Bearer ")) {
                response.setMessage("Token no proporcionado o formato inválido");
